...
public class AccountApiControllerTest {
    ...
    public void testCreateAccount() throws Exception {
        ...
        user.setRole("USER");
        user.setUserID(3L); // Use a valid user ID

        // Mock the UserRepository to return the created User
        when(userRepository.findById(3L)).thenReturn(Optional.of(user));

        // Create an AccountDTO object with the User included
        AccountDTO accountDTO = new AccountDTO();
        accountDTO.setAccountNumber("12345678");
        accountDTO.setAccountType("Savings");
        accountDTO.setUser(user); // Include the User in AccountDTO

        // Create a mock Account object
        Account mockAccount = new Account();
        mockAccount.setAccountID(1L); // Set a mock account ID
    ...
    public void testDeleteAccount_Success() throws Exception {
        ...
        doNothing().when(balanceRepository).deleteById(1L);
